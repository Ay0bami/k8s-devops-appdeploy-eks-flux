bash-4.4# cat /usr/local/bin/docker-entrypoint.sh 
#!/bin/bash
set -eo pipefail
shopt -s nullglob
...
docker_setup_db() {
        ...
        if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_PASSWORD" ]; then
                ...
                # Kamaal says let dbadmin access the DB!
                GRANT CREATE, ALTER, INSERT, UPDATE, DELETE, SELECT on employees.* TO '$MYSQL_USER'@'localhost';
